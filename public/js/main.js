...
APP.init = function(){
	...
	// ensure missing images get a visible fallback instead of staying broken
	$(document).on("error", "img", function(){
		var $img = $(this);
		if($img.data("fallback")){ return; }
		$img.data("fallback", true);
		$img.attr("src","images/background.jpg");
	});
...
APP.go = function(state, storeHistory){
	...
	switch(section){
		case "home":
			...
			if(storeHistory) { history.pushState(state, title, url); }
			APP.state = state;
			APP.webGL.go("home");
			APP.home.show();
			break;
