...
var FALLBACK_FRAGMENT_SHADER = [
  ...
  "  vec3 top = vec3(0.1, 0.13, 0.15);",
  "  vec3 bottom = vec3(0.04, 0.05, 0.06);",
  "  vec3 color = mix(bottom, top, gradient);",
...
APP.webGL = {
  ...
  paused:false,
  intensity:1.0,
  createFallbackTexture:function(){
  ...
  init : function(){
        ...
        */
        ...
        var placeholderTexture = this.createFallbackTexture();
        $.each(APP.data.textures,function(i,t){
          loader.load( t.file, function (object) {
                APP.webGL.textures[t.name] = object;
              }, undefined, function(){
                console.warn("Falling back placeholder texture for", t.file);
                var fallback = placeholderTexture.clone ? placeholderTexture.clone() : placeholderTexture;
                APP.webGL.textures[t.name] = fallback;
              });
        });
...
/*
        ...
        */
        ...
        setTimeout(function(){
          if(!APP.modelsLoaded){
            console.warn("Forcing preload completion without external assets.");
            APP.webGL.createBackgroundShader(FALLBACK_VERTEX_SHADER, FALLBACK_FRAGMENT_SHADER);
          }
        },2000);
  ...
  resize : function(){
    ...
    if(!_this.backgroundPlane || !_this.backgroundUniforms){ return; }
  ...
  createBackgroundShader : function(v,f){
    ...
    _this.backgroundUniforms = {
          ...
          iChannel0:  { type: 't', value: _this.textures['tex1'] || _this.createFallbackTexture() }
    };
    ...
    if(_this.backgroundPlane){
      _this.scene.remove(_this.backgroundPlane);
    }
...
function ShaderLoader(vertex_url, fragment_url, onLoad, onProgress, onError) {
    ...
    vertex_loader.load(vertex_url, function (vertex_text) {
        loadFragment(vertex_text || FALLBACK_VERTEX_SHADER);
    }, onProgress, function(err){
        console.warn("Vertex shader load failed, using fallback.", err);
        loadFragment(FALLBACK_VERTEX_SHADER);
    });

    function loadFragment(vText){
        var fragment_loader = new THREE.FileLoader(THREE.DefaultLoadingManager);
        fragment_loader.setResponseType('text');
        fragment_loader.load(fragment_url, function (fragment_text) {
            onLoad(vText, fragment_text);
        }, onProgress, function(err){
            console.warn("Fragment shader load failed, using fallback.", err);
            onLoad(vText || FALLBACK_VERTEX_SHADER, FALLBACK_FRAGMENT_SHADER);
        });
    }
}
